# AGENTS.md — money-tracker

## Project structure

Rust workspace with three crates plus a frontend package:
- `money_core/` — library: SQLite (rusqlite bundled, no external deps), models, services. Pure model —
  no printing, no prompting, no argument parsing, no presentation formatting. Depends on nothing
  CLI/GUI-specific (`cargo tree -p money_core` must never show `clap`/`dialoguer`/`tabled`/`tauri`).
- `cli/` — binary: clap derive commands + dialoguer interactive prompts. A thin handler over
  `money_core`.
- `gui/src-tauri/` (crate name `gui`) — Tauri v2 backend: thin `#[tauri::command]` wrappers, one per
  `money_core` service call, plus `AppState` (a long-lived `Mutex<Connection>`) and `ApiError`
  (`AppError` isn't `Serialize`, so every command returns `ApiError` instead). `gui/src/` — React + TS
  frontend, CSS Modules, no router/query library (IPC is local, so a global revision counter in
  `hooks/useApi.ts` triggers refetch after every mutation).

## Build, run & test

```sh
cargo build --workspace          # compiles money_core + cli + gui/src-tauri
cargo run -p money-tracker -- <command>   # runs the CLI (e.g. report -p 2026-07)
cargo test --workspace           # 39 tests: unit tests per service + money_core/tests/scenarios.rs
cargo clippy --workspace --all-targets

cd gui && pnpm install && pnpm tauri dev   # runs the GUI as a native window
npx tsc --noEmit                            # type-check the frontend (run from gui/)
```

Use `MONEY_TRACKER_DB=/path/to/file.db` to point at a throwaway database instead of
`~/.money-tracker/data.db` — essential for tests and manual verification. Both the CLI and the GUI
honor it, and both can run concurrently against the same file (WAL mode).

Remote (Supabase) mode is opt-in: `MONEY_TRACKER_SUPABASE_URL` + `MONEY_TRACKER_SUPABASE_KEY` (or
`~/.money-tracker/config.toml`, which `db remote login` writes). In remote mode the same local file
becomes a read-only mirror of Supabase; `production_backend(&Settings::load())` in
`money_core::sync` returns the sync-wrapped backend (auth via keyring-stored refresh token), and
without remote config it returns the plain local `SqliteBackend`. See `db remote status` /
`db remote sync` / `db remote migrate`.

### Regenerating GUI type bindings

Models consumed by the GUI derive `ts_rs::TS` behind an opt-in `ts-rs` feature on `money_core`
(off by default so the CLI-only build doesn't pull in the codegen macro):

```sh
cargo test -p money_core --features ts-rs   # writes gui/src/bindings/*.ts
```

Two things to know if you add a new exported field: `ts-rs`'s `export_to` path is relative to
`<crate>/bindings/`, not the crate root — hence the `"../../gui/src/bindings/"` (not `"../gui/..."`)
in every `#[ts(export_to = ...)]` attribute. And `i64`/`Option<i64>` fields need an explicit
`#[ts(type = "number")]` / `#[ts(type = "number | null")]` override — ts-rs's default `i64 -> bigint`
mapping doesn't match what actually arrives over Tauri's `serde_json`-based IPC (a plain JS `number`).

## Project layout

```
Cargo.toml            # workspace root (money_core + cli)
money_core/
  src/
    db.rs              # connection, schema (accounts/entries/budgets/concepts/config), user_version gate
    period.rs           # Period ("YYYY-MM") + today()/validate_date() — single source of truth for dates
    settings.rs         # Settings (supabase_url/key) from config.toml + env, mirror_path
    auth.rs             # SupabaseAuth (GoTrue): login/refresh, keyring/0600 refresh-token persistence
    sync/mod.rs         # MirroringBackend (remote + mirror), production_backend, migrate_local_to_remote
    storage/            # LedgerBackend trait (mod.rs) + SqliteBackend (sqlite.rs) + SupabaseBackend (remote.rs)
    models/             # AccountKind, NewAccount, AccountBalance, EntryKind, NewEntry, Entry,
                         # ConceptSummary, Budget, Concept, Config
    services/
      account_service.rs   # create/list/archive/reconcile accounts
      entry_service.rs     # sole writer of `entries`; income/expense/transfer/opening + emergency split
      report_service.rs    # monthly_report, net_worth, full_status
      setup_service.rs     # opening-balance seeding
  tests/scenarios.rs   # black-box tests through the public API only
cli/
  src/
    main.rs
    commands/          # add, income, transfer, bucket, account, entry, concept, budget, report,
                        # config, setup, db (incl. `db remote login|logout|status|sync|migrate`), helpers
gui/
  src/                 # React + TS frontend
    bindings/          # generated by ts-rs — do not hand-edit
    api/                # typed wrappers over Tauri's invoke(), one module per command group
    hooks/useApi.ts     # fetch-on-mount + global revision counter for refetch-after-mutation
    hooks/useSync.ts    # polls sync_poll every ~30 s; bumps revision only when the mirror cursor moved
    routes/             # Dashboard, Register, Accounts, Movements, Budgets, Settings (+ login/sync panel), SetupWizard
  src-tauri/
    src/
      state.rs          # AppState { backend: Mutex<Box<dyn LedgerBackend>>, session_email }
      error.rs          # ApiError, From<AppError> (includes remote/network/auth variants)
      commands/         # one file per command group, mirrors money_core's services/; sync.rs = poll/status/login/logout
Dashboard_Financiero.xlsx / .ods   # LEGACY dashboard, read-only reference — no longer imported
```

## Data model

Everything is a movement between accounts, or across the system boundary. Two tables carry the whole
model:

```
accounts(id, name UNIQUE, kind CHECK IN ('spending','emergency','target','credit'),
         target_amount, credit_limit, liquid, archived)

entries(id, date, kind CHECK IN ('income','expense','transfer','opening'),
        amount CHECK(amount > 0),           -- always positive; kind gives direction
        from_account_id, to_account_id,     -- exactly one of these two shapes, enforced by CHECK:
        concept, subconcept, description)   --   income/opening: to only · expense: from only · transfer: both
```

Account balances are **derived**, never stored: `account_balances` is a view computing
`SUM(to) - SUM(from)` per account. There is no `UPDATE ... current_balance` anywhere, so balances
cannot desync from the ledger. This is also what makes a seeded balance carry forward across months —
there is no month-scoped "flujo" number, only a query with no date filter.

Key rules baked into the schema, not just convention:
- At most one active `emergency` account (partial unique index on `kind='emergency' AND archived=0`).
- Only `target` accounts may have a `target_amount` (CHECK) — but it's optional even there, for an
  open-ended accumulation bucket (e.g. "Patrimonio") with no specific goal. `AccountBalance::progress_pct()`
  returns `None` when it's unset, and every caller (CLI, GUI) already renders that as "—".
- `kind='transfer'` is the only path that can move money between two accounts without touching
  income/expense totals — this is what lets a credit-card payment, a bucket withdrawal, or an ATM
  cash-envelope withdrawal happen without being double-counted as spending.
- `kind='opening'` is shape-identical to `income` but structurally excluded from income totals — this
  is what a seeded starting balance uses, so `setup` never inflates the month it runs in.
- `accounts.liquid` gates the automatic emergency-fund split on `income`: an income landing in a
  non-liquid account (e.g. "vales de despensa", which can't physically be moved to savings) never
  triggers the split.

## Reporting

Because an expense can be paid from a spending account, a credit card, or straight out of a savings
bucket, "how much did I spend this month" has two different honest answers, both shown by `report`:

- **Gasto del mes (devengado)** — everything consumed this month, regardless of funding source. This
  is what budgets compare against.
- **Salida real de efectivo** — what actually left spending accounts, including any card payments made
  this month (which fund nothing new, they just settle a prior month's charge).

`report --detail` breaks the devengado figure down into paid-with-flow / funded-from-savings /
on-credit, and always shows savings contributions/withdrawals and card payments as separate lines
when nonzero.

## CLI commands

All commands take flags for scripted/non-interactive use, or fall back to dialoguer prompts. Every
command that registers money resolves one of three modes (`cli/src/commands/helpers.rs::PromptMode`):
`Wizard` (no args, or `-i`) prompts for everything including optionals; `Fill` (some args) prompts only
for missing required fields; `Strict` (`--yes`) never prompts, errors instead.

- `add <AMOUNT> <CONCEPT> [--from ACCOUNT]` — register an expense
- `income <AMOUNT> <CONCEPT> [--to ACCOUNT] [--no-emergency]` — register an income; auto-splits
  `emergency_pct`% into the emergency account unless the destination is non-liquid or `--no-emergency`
- `transfer -a AMOUNT --from ACCOUNT --to ACCOUNT` — move money between any two accounts (card
  payments, ATM withdrawals, ad-hoc moves)
- `bucket deposit -b BUCKET -a AMOUNT [--from ACCOUNT]` / `bucket withdraw -b BUCKET -a AMOUNT [--to ACCOUNT]`
  — sugar over `transfer` for savings buckets; `withdraw` explicitly prints that it is NOT an expense
- `account add <NAME> --kind <spending|emergency|target|credit> [--target N] [--limit N] [--restricted]`
  / `account list [--all]` / `account archive <NAME> [--force]` — buckets ARE accounts; there is no
  separate bucket-creation command
- `account reconcile <NAME> --actual N [-c CONCEPT]` — the cash-envelope flow: writes the adjusting
  expense/income needed to bring a derived balance to what was physically counted
- `entry list [-p PERIOD] [-c CONCEPT] [--account NAME] [--kind K]` / `entry rm <ID>` — inspect/correct
  individual ledger rows
- `budget set -c CONCEPT -l LIMIT [-p PERIOD]` / `budget show [-p PERIOD]` / `budget rm -c CONCEPT` —
  informative only; never blocks an over-budget expense
- `report [-p PERIOD] [--detail]`
- `setup [--account NAME=AMOUNT ...] [-D DATE]` — load opening balances into a fresh (or `--force`d)
  database, as `kind='opening'` entries
- `db status` / `db reset [--backup]` / `db remote login|logout|status|sync|migrate` — the `remote`
  subcommands manage Supabase mode: `login` writes `supabase_url`/`supabase_publishable_key` to
  `config.toml` and stores the refresh token in the OS keyring (0600 file fallback); `status` shows
  mode/session/remote revision/mirror watermark; `sync` pulls remote changes into the mirror once;
  `migrate [--force]` pushes the whole local DB to Supabase (remapping ids) and rebuilds the mirror
- `concept list|add`, `config get|set|list`

Config keys used: `emergency_pct` (default 10), `default_account` (fallback for `add --from`),
`income_account` (fallback for `income --to`), `cash_concept` (default concept for
`account reconcile`).

## Legacy dashboard

`Dashboard_Financiero.xlsx`/`.ods` is a **read-only reference** from the previous Excel-based workflow.
It is not imported — the ODS importer was deleted along with the old `transactions`/`buckets` schema
it targeted (it had several real bugs: `Extraordinario` misclassified as income, footnote text scraped
into `concepts`, hardcoded April–July month mapping). A fresh database starts empty; `setup` loads the
two numbers that matter (current emergency-fund balance, current cash) and everything else is entered
going forward through the CLI.

## Schema versioning

`open_db()` gates on `PRAGMA user_version`. A database created by the pre-redesign build (detected by
the presence of the old `transactions`/`buckets` tables) is refused with an actionable error rather
than silently migrated — there is no automatic migration path from the old schema. See
`money_core::error::AppError::LegacySchema`.
